...
Se $\underline{Y}$ è funzione deterministica di $\underline{X}$, allora
$$H(\underline{X}, \underline{Y})=H(\underline{X})$$
Dim:
$$\begin{align*}
	H(\underline{X},\underline{Y})
	&=\sum_{\underline{a}\in[a_1,a_2]}p_{x,y}(a_1,a_2)\log_2\big(\frac{1}{p_{x,y}(a_1,a_2)}\big)\\
	&=\sum_{a_1\in A_x}p_{x,y}(a_1,f(a_1))\log_2\big(\frac{1}{p_{x,y}(a_1,f(a_1))}\big)\\
	&=\sum_{a_1\in A_x}p_x(a_1)\log_2\big(\frac{1}{p_x(a_1)}\big)\\
	&=H(\underline{X})
\end{align*}$$
