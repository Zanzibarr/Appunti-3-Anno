...
\subsection{Entropia di una variabile aleatoria}
L'entropia di un vettore aleatorio, come per una variabile aleatoria discreta, è $$\underline{H}(\underline{X})=E\big[i_{\underline{x}}(\underline{X})]\big]=\sum_{\underline{a}\in A}p_{\underline{x}}(\underline{a})\log_2\big(\frac{1}{p_{\underline{x}}(\underline{a})}\big)$$

\subsection{Entropia di più variabili aleatorie}
Consideriamo due v.a. $\underline{X}$ $\underline{Y}$.\\
\begin{enumerate}
	\item Se $\underline{X}$ e $\underline{Y}$ sono \textbf{indipendenti}: $$H(\underline{X},\underline{Y})=E\big[i_{x,y}(\underline{X},\underline{Y})\big]=E\big[i_x(a)+i_y(b)\big]=H(\underline{X})+H(\underline{Y})$$
	\item Se $\underline{X}$ e $\underline{Y}$ \textbf{non sono indipendenti}: $$H(\underline{X},\underline{Y})<H(\underline{X})+H(\underline{Y})$$
	\item Se $f(\underline{X})=\underline{Y}$ è \textbf{funzione deterministica}: $$H(\underline{X}, \underline{Y})=H(\underline{X})$$
	La cosa tuttavia non ci sorprende in quanto l'entropia esprime l'incertezza sulla coppia, e essendo $\underline{Y}$ funzione deterministica di $\underline{X}$, l'incertezza della coppia è quella di $\underline{X}$.\\
	Attenzione che non vale in generale il contrario, ovvero $$H(\underline{X},\underline{Y})\neq H(\underline{Y})$$
	\item Se $\underline{Y}$ \textbf{non è funzione deterministica} di $\underline{X}$: $$H(\underline{X},\underline{Y})>H(\underline{X})$$	
	\item Se $f(\underline{X})=\underline{Y}$ è \textbf{funzione iniettiva}, la densità di probabilità di $\underline{X}$ è la stessa di $\underline{Y}$ a meno di una modifica dell'alfabeto: $$H(\underline{X})=H(\underline{Y})=H(\underline{X},\underline{Y})$$
\end{enumerate}
Volendo raccogliere tutte le diseguaglianze ricavate, abbiamo la seguente catena:
$$0\leq max\{H(\underline{X},\underline{Y})\}\leq H(\underline{X},\underline{Y})\leq H(\underline{X}+\underline{Y})$$


TODO RECUPERO APPUNTI DELLA LEZIONE 06/12 MINUTO 52\\
TODO LEZIONE 07/12
