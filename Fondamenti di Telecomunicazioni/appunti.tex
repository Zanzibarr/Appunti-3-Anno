...
con $\underline{\epsilon}_{min}(\underline{\sigma})$ il \textbf{coset leader} della sindrome $\underline{\sigma}$.
...
$$I_s(\underline{X};\underline{Y})=H_s(\underline{X})+H_s(\underline{Y})-H_s(\underline{X},\underline{Y})=H_s(\underline{Y})-H_s(\underline{Y}|\underline{X})$$
...
La capacità del canale è il \textbf{massimo tasso di informazione del canale} tra tutti quelli ottenuti, usando tutte le possibili distribuzioni della sorgente $$C=max\big(R(\mathcal{G})\big)=\frac{1}{T}max\{H_s(Y)-H_s(Y|X)\}$$
...
Con $\frac{k}{n}$ il \textbf{rate del codice} o \textbf{tasso del codice}.
...
Serve a fornire un codice a prefisso, non per forza ottimo (ma di più facile implementazione).\\
Data la densità di probabilità delle parole d'ingresso: $$\mathcal{D}_n=\{a_1,...,a_N\},\quad p_n(a_1),...,p_n(a_N)$$
Disponiamo le probabilità $p_n$ in sequenza su un segmento lungo $1$ e indichiamo con $m_i$ i punti medi nei segmenti delle probabilità.\\
\begin{center}
	\begin{tikzpicture}[scale = 3]
		\draw[->] (-1,0) -- (2,0) node[right]{$\phi_1$};
		
		\draw[color=black] (-0.5,-0.03) -- (-0.5,0.03);
		\node[above] at (-0.5,0) {$0$};
		
		\draw[color=black] (1.5,-0.03) -- (1.5,0.03);
		\node[above] at (1.5,0) {$1$};
		
		\draw[color=black] (0,-0.03) -- (0,0.03);
		\node[below] at (-0.25,0) {$p_n(a_1)$};
		
		\draw[color=black] (0.5,-0.03) -- (0.5,0.03);
		\node[below] at (0.25,0) {$p_n(a_2)$};
		
		\draw[color=black] (0.9,-0.03) -- (0.9,0.03);
		\node[below] at (0.7,0) {$p_n(a_3)$};
		
		\node[below] at (1.2,0) {$p_n(a_4)$};
		
	
		\node[red] at (-0.25,0) {$\times$};
		\node[above] at (-0.25,0) {$m_{a_1}$};
		\node[red] at (0.25,0) {$\times$};
		\node[above] at (0.25,0) {$m_{a_2}$};
		\node[red] at (0.7,0) {$\times$};
		\node[above] at (0.7,0) {$m_{a_3}$};
		\node[red] at (1.2,0) {$\times$};
		\node[above] at (1.2,0) {$m_{a_4}$};
		
	\end{tikzpicture}
\end{center}

Abbiamo quindi che $m_1<m_2<...<m_N$.\\
Convertiamo i numeri in binario $m_{a_i}=0.0010..01$ e per ogni parola $a_i$ calcoliamo la lunghezza della sequenza: $L_i=\lceil i(a_i)+1\rceil$.\\
Prendiamo quindi la parte dopo la virgola di $(m_{a_i})_{2}$ e consideriamo solo i $L_i$ bit più significativi.\\
Andremo così a creare il codice di Elias che avrà una lunghezza media $L_y\geq L_y(ottimo)$.\\
...
\begin{enumerate}
	\item Per $M_y=2$ il limite inferiore è vicino a $1^-$
